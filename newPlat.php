...
if (!empty($_POST["plantName"])) {
    ...
    $query = "INSERT INTO plant (user_id, urgency, summary , _type, _date , plant_name,pic_path)
...
if ($_SESSION["user_type"] == 'farmer') {
    $query = "SELECT MAX(case_id) AS lastCaseId FROM plant WHERE user_id = '" . $_SESSION["user_id"] . "'";
    $result = mysqli_query($connection, $query);
    if (!$result) {
        header("Location: login.php");
        die("DB query failed.");
